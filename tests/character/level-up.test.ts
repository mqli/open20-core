...
describe('levelUp', () => {
  ...
  it('7. new resources: Fighter 1 → 2 (Action Surge)', () => {
    ...
    expect(char.resources.some(r => r.id === 'Second Wind')).toBe(true);
    expect(char.resources.some(r => r.id === 'Action Surge')).toBe(false);

    const result = levelUp(char, { classId: 'Fighter', hpChoice: 'fixed' }, data);

    // Should still have Second Wind
    expect(result.resources.some(r => r.id === 'Second Wind')).toBe(true);
    // Should now have Action Surge
    expect(result.resources.some(r => r.id === 'Action Surge')).toBe(true);
  });
  ...
  it('does not duplicate existing resources', () => {
    ...
    expect(char.resources.filter(r => r.id === 'Second Wind')).toHaveLength(1);
