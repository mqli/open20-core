...
function createMockDataLoader(): DataLoader {
  const weapons: Record<string, Weapon> = {
    ...
    Dagger: {
      ...
      cost: '2 gp',
      equipped: false,
      damage: { dice: 'd4', ability: 'Strength', bonus: 0 },
    ...
    Shortbow: {
      ...
      cost: '25 gp',
      equipped: false,
      damage: { dice: 'd6', ability: 'Dexterity', bonus: 0 },
    ...
    Quarterstaff: {
      ...
      cost: '2 sp',
      equipped: false,
      damage: { dice: 'd6', ability: 'Strength', bonus: 0 },
      properties: ['Versatile'],
      versatileDamage: 'd8',
      mastery: 'Sap',
    },
...
describe('calculateAttacks', () => {
  ...
  it('should ensure damage modifier is never negative (2024 rules)', () => {
    const scores = makeScores(8); // Str 8 → mod -1, but should be 0 in damage
    const equipment = [makeEquippedWeapon('Longsword')];
    const result = calculateAttacks(scores, equipment, 3, emptyFeatures, mockData);
